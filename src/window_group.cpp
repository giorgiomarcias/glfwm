...
namespace glfwm {
    ...
    void WindowGroup::destroy()
    {
        ...
        std::deque<WindowGroupID>::iterator pos = std::lower_bound(freedWindowGroupIDs.begin(), freedWindowGroupIDs.end(), groupID);
     ...
     */
    WindowGroupID WindowGroup::getID() const
    {
        return groupID;
    }
     ...
     */
    bool WindowGroup::isPolling() const
    {
        return doPoll;
    }

    /**
     *  @brief  The runLoopConcurrently method starts the execution of this group loop in another thread, if not yet started.
     ...
     *  @note   This does not sinchronize, just tells the thread to stop and returns immediately. See stopAndWait if you need to sinchronize.
     */
    ...
    /**
     *  @brief  The stopAndWait method breaks the executiotn of the loop running on another thread and sinchronizes with its ending.
     */
    ...
    /**
     *  @brief  The concurrentLoop method is the function to be executed on another thread and that represents a loop of event processing and drawing.
     */
    ...
    /**
     *  @brief  The process method makes this group draw the windows in windowsToUpdate or all the attached windows (if doPoll),
     *          directly or indirectly (if running concurrently).
     */
    ...
    void WindowGroup::updateWindows()
    {
        ...
        if (
            ...
            doPoll ||
#endif
            windowsToUpdate.find(WholeGroupWindowIDs) != windowsToUpdate.end() || windowsToUpdate.find(AllWindowIDs) != windowsToUpdate.end())
            for (auto id : attachedWindows) {
    ...
    /**
     *  @brief  The newGroup static method allocates and creates a new WindowGroup, storing also information for later retrieval.
    ...
    /**
     *  @brief  The getAllUngroupedWindowIDs static method returns the set of all the WindowIDs that are not attached to any group.
