...
namespace glfwm {
     ...
     */
    void WindowManager::setSwapInterval(const int intervals)
    {
     ...
     */
    void WindowManager::resetDefaultHints()
    {
     ...
     */
    WindowPointer WindowManager::createWindow(const int width, const int height, const std::string &title, const EventBaseType eventTypes,
                                              GLFWmonitor *monitor, const WindowPointer &share)
    {
     ...
     *    @return A pointer to the new created Window.
     */
    WindowPointer WindowManager::createWindow(const int width, const int height, const std::string &title, const EventType eventType,
                                              GLFWmonitor *monitor, const WindowPointer &share)
    {
    ...
    void WindowManager::inputMouseButtonCallback(GLFWwindow *glfwWindow, int button, int action, int mods) {
        ...
        // if found, make it handle the event
        EventPointer emb = std::make_shared<EventMouseButton>(wID, static_cast<MouseButtonType>(button), static_cast<ActionType>(action), mods);
        WindowPointer w = Window::getWindow(wID);
        if (w) {
            w->makeContextCurrent();
            w->handleEvent(emb);
    ...
    void WindowManager::inputKeyCallback(GLFWwindow *glfwWindow, int key, int scancode, int action, int mods) {
        ...
        // if found, make it handle the event
        EventPointer ek = std::make_shared<EventKey>(wID, static_cast<KeyType>(key), static_cast<char32_t>(scancode), static_cast<ActionType>(action), mods);
        WindowPointer w = Window::getWindow(wID);
        if (w) {
            w->makeContextCurrent();
            w->handleEvent(ek);
